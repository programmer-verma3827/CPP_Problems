...
pair<int, int> getMinAndMax(BinaryTreeNode<int> *root) {
    ...
    if(root->left != NULL)
    {
        ...
        p.first = min(p.first, leftResult.first);
        p.second = max(p.second, leftResult.second);
    }
    
    if(root->right != NULL)
    {
        pair<int, int> rightResult = getMinAndMax(root->right);
        p.first = min(p.first, rightResult.first);
        p.second = max(p.second, rightResult.second);
    }
